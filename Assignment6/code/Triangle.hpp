                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    auto s2de2 = dotProduct(s2, e2);
    auto s1ds = dotProduct(s1, s);
    auto s1dir = dotProduct(s2, dir);

    auto s1de1 = dotProduct(s1, e1);

    if (s1de1 == 0) {
        return false;
    }

    auto tb1b2 = Vector3f(s2de2, s1ds, s1dir) / s1de1;
    auto t = tb1b2.x;
    auto b1 = tb1b2.y;
    auto b2 = tb1b2.z;
    tnear = t;
...
inline Bounds3 Triangle::getBounds() { return expand(Bounds3(v0, v1), v2); }
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    auto s2de2 = dotProduct(s2, e2);
    auto s1ds = dotProduct(s1, s);
    auto s1dir = dotProduct(s2, ray.direction);
